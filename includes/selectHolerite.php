<div class="container-fluid">
	...
	<?php
		...
		if(mysqli_query($conn, $sql_holerite)) {
			...
			$dataCredito = substr($data_bd,2,2)."/".substr($data_bd,0,2)."/".substr($data_bd,4,4);
			//seleciona dados da tabela total_final
			$sql_holerite = "SELECT * FROM total_final WHERE fk_id_holerite = '$id_holerite'";
			$result = mysqli_query($conn, $sql_holerite);
			$row = mysqli_fetch_array($result, MYSQL_ASSOC);
			...
			<!-- Inicio da tabela do holerite para apresentação e impressão -->
				...
				<div align=right><b> Data do Crédito: </b><?php echo $dataCredito; ?></div>
				...
				<tr><td><?php echo $user_name; ?></td><td colspan='2'><?php echo $nome; ?></td><td><?php echo $cpf_mask; ?></tr>
				<?php
				if ($cargo<>''){?>
					<thead style="font-weight:bold;border-top: 1px solid #D5D5D5;border-bottom: 1px solid #D5D5D5;" bgcolor="#D5D5D5"><td colspan='4'>Cargo</td></thead>
					<tr><td><?php echo $cargo;?></td></tr>
				<?php
				}
				?>
